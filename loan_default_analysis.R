...
comparison_table <- data.frame(
  ...
  Logistic_Regression = c(86.66, 49.16, 93.70, 64.49, 98.92, 0.8429),
  Decision_Tree       = c(87.61, 55.32, 90.81, 68.75, 98.17, 0.7845),
  Random_Forest       = c(88.62, 57.79, 93.55, 71.44, 98.70, 0.8846),
  XGBoost             = c(89.68, 62.92, 92.91, 75.03, 98.43, 0.8961)
)
...
#PR Curve all models
pr_lr  <- pr.curve(scores.class0 = lr_probs,  weights.class0 = as.numeric(as.character(test$Status)), curve = TRUE)
pr_dt  <- pr.curve(scores.class0 = dt_probs,  weights.class0 = as.numeric(as.character(test$Status)), curve = TRUE)
pr_rf  <- pr.curve(scores.class0 = rf_probs,  weights.class0 = as.numeric(as.character(test$Status)), curve = TRUE)
pr_xgb <- pr.curve(scores.class0 = xgb_probs, weights.class0 = as.numeric(as.character(test$Status)), curve = TRUE)
pr_data <- rbind(
  data.frame(
...
ggplot(comparison_long, aes(x = Metric, y = Value, fill = Model)) +
  ...
  geom_text(aes(label = paste0(Value, "%")),
            ...
            vjust = -0.4, size = 2.8) +
  scale_fill_manual(values = c("Logistic_Regression" = "#e74c3c",
                               "Decision_Tree"       = "#2ecc71",
                               "Random_Forest"       = "#9b59b6",
                               "XGBoost"             = "#f39c12"),
                    labels = c("Decision Tree", "Logistic Regression",
